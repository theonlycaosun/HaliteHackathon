...
Move assignMove(Location t) {
    ...
    for(int direction : CARDINALS) {
        if (presentMap.getSite(presentMap.getLocation(t, static_cast<unsigned char>(direction))).owner != myID && presentMap.getSite(t).strength > presentMap.getSite(presentMap.getLocation(t, static_cast<unsigned char>(direction))).strength) {
            return Move{t, static_cast<unsigned char>(direction)};
        }
    }
    if (presentMap.getSite(t).strength < presentMap.getSite(t).production*8) {
        return Move{t, STILL};
    }
    else {
